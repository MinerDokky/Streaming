...
<img class="films" src="poster/vikings.jpg">
<?php
$requete = $bdd->query("SELECT titre FROM serie WHERE id_s = 1 AND id_c = 1");
if($reponse = $requete->fetch())
{
	$titre = $reponse['titre'];
}
$requete = $bdd->query("SELECT description FROM serie WHERE id_s = 1 AND id_c = 1");
if($reponse = $requete->fetch())
{
	$description = substr($reponse['description'],0,60)."... <a href='serie/watch.php?titre=Vikings&ids=1&idsn=1&ep=1'>Regarder</a>";
