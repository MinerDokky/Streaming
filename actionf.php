...
<img class="films" src="poster/logan.jpg">
<?php
$requete = $bdd->query("SELECT titre FROM film WHERE id_f = 1 AND id_c = 1");
if($reponse = $requete->fetch())
{
	$titre = $reponse['titre'];
}
$requete = $bdd->query("SELECT description FROM film WHERE id_f = 1 AND id_c = 1");
if($reponse = $requete->fetch())
{
	$description = substr($reponse['description'],0,60)."... <a href='films/watch.php?titre=logan&idf=1'>Regarder</a>";
